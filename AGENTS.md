...
- When the user asks to create a GitHub issue, read `harness/rules/github-issue-creation-rules.md`, display the exact repository, title, body, and metadata, and obtain explicit current-user confirmation before submission.

## Workflow Tiers
- `standard`: after the common proposal is approved, use one `docs/changes/<change>.md` from `docs/changes/_template.md`, then implement, verify proportionately, complete the record, and write an independently defect-reviewed `completion-report.md` before handoff.
- `high-risk`: after proposal/tier confirmation, complete design, implementation, testing, independent acceptance, and closeout under `harness/rules/task-execution-rules.md`.
- `general` is the fallback routing stage only for standard analysis, diagnosis, explanation, internal documentation, or non-behavioral maintenance that does not fit a high-risk responsibility stage. High-risk work must use proposal, design, implementation, testing, or acceptance.
...
- A request to create, file, open, publish, or submit a GitHub issue activates `harness/rules/github-issue-creation-rules.md`; issue submission waits for confirmation of the displayed final content even when another workflow approval already exists.
...
2. `harness/rules/task-execution-rules.md` controls continuous execution after proposal confirmation.
...
9. Only after confirmation, reroute from proposal to the selected execution responsibility and run the final tier flow: one change record plus a lightweight completion report for standard, or the expanded packet and full staged lifecycle for high-risk.
...
- Smart Approvals `guardian` does not replace proposal confirmation, stage checks, or independent Harness acceptance.
...
## Rule Ownership
- `task-execution-rules.md` owns continuous execution after proposal confirmation, automatic stage advancement, correction loops, and closeout.
...
- `harness/scripts/harness-check.py` runs one high-risk stage's checks; `task-transition.py` owns legal stage changes and durable receipts, while `lifecycle-check.py` owns prior-stage/final completeness. `lower-tier-check.py` owns the required standard baseline, changed-path manifest, proposal-entry validation, and proportional defect-discovery report check. `stage-scope-check.py` rejects out-of-stage paths; `schema-check.py` remains an internal entrypoint.
...
- `github-issue-creation-rules.md` exclusively owns the final-content confirmation gate before a GitHub issue is submitted.
