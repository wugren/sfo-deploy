...
- 一次部署执行为每台目标机器缓存一个远程会话；OpenSSH 通过该会话专用的本地控制套接字复用底层连接，
  后续 `ssh` 命令与 `scp` 上传共用主连接。会话关闭时退出主连接并删除控制套接字；主连接异常中断时
  OpenSSH 可重新建连，闲置主连接至多保留 30 分钟以避免控制端异常退出后长期驻留。
- 每台机器的可选 `enable_deno` 布尔字段缺省为 `true`，控制是否允许执行远端 Deno。`plan` 只读本地
  配置，不连接 SSH 或依据 OS 名称推断运行时；显式禁用时若选中步骤需要 Deno 脚本或真实秘密配置
  渲染，计划和执行入口在远端部署写入前失败。启用时按实际步骤预检目标 Deno 2，不支持则报错。
  内置版本化发布与无秘密受管配置经 SSH 文件操作交付，无需远端 Deno。归档计划保存机器策略，旧归档
  缺失字段时视为启用。`${APP_VERSION}` 和普通参数由控制端渲染，三个安装目录变量用于配置目标路径；
  `APP_VERSION` 若显式声明为秘密，则按秘密处理。
- `install-deno` 对 `enable_deno: false` 的选中机器在连接前跳过并输出 info 日志；确认、进度和结果只覆盖实际处理机器。回滚仍按归档机器策略执行。
